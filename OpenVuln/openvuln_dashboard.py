...
def main():
    ...
    if metrics_df is not None and df is not None:
        ...
        # === Throughput vs Cost Bubble Chart ===
        st.subheader(f"💰 Throughput vs Cost — Bubble Chart [{selected_type}]")
        st.markdown("Bubble size represents **F1 score** — larger bubbles = better performance.")
        
        # Hardcoded throughput & cost data (from benchmarks)
        MODEL_BENCHMARKS = {
        ...
        # Merge metrics_df with benchmark data
        # Normalize model names: OpenVuln uses underscores (from folder names), benchmarks use dashes
        def normalize_model_name(name):
            return name.replace('_', '-')
        
        # Build lookup with normalized keys
        benchmarks_normalized = {normalize_model_name(k): v for k, v in MODEL_BENCHMARKS.items()}
        
        bubble_rows = []
        for _, row in metrics_df.iterrows():
            model_name = row['Model']
            normalized = normalize_model_name(model_name)
            if normalized in benchmarks_normalized:
                bm = benchmarks_normalized[normalized]
                bubble_rows.append({
                    'Model': model_name,
        ...
        if bubble_rows:
            ...
            for r in bubble_rows:
                if f1_max > f1_min:
                    norm = (r['F1 Score'] - f1_min) / (f1_max - f1_min)
                else:
                    norm = 0.5
                r['Bubble Size'] = 5 + (norm ** 2) * 75  # Squared scaling: 5 (worst) to 80 (best)
        ...
        if bubble_rows:
            ...
            fig_bubble = px.scatter(
                ...
                title=f"Latency vs Cost (Bubble Size = F1 Score) [{selected_type}]",
                size_max=60,
            ...
            st.plotly_chart(fig_bubble, use_container_width=True, key='openvuln_latency_cost_bubble')
            ...
            st.info("No models with benchmark data found in current selection.")
